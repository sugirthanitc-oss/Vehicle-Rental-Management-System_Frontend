...
import { Zap, Car, User, LogOut, LayoutDashboard, Menu, X, Shield, ChevronDown, Building2, Sun, Moon, Edit3 } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 glass-nav transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            {/* Right Action / Theme Switcher / Auth State */}
            <div className="hidden md:flex items-center space-x-3">
              
              {/* Top-Right Theme Toggle Button */}
              <button
                onClick={toggleTheme}
                className="p-2.5 rounded-2xl glass-card text-slate-300 hover:text-amber-400 transition-colors border border-slate-700/60 flex items-center justify-center shadow-sm"
              ...
              {isAuthenticated ? (
                <div className="relative">
                  ...
                  {/* Dropdown Menu */}
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-64 rounded-2xl glass-card py-2 border border-slate-700/60 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                      <div className="px-4 py-2.5 border-b border-slate-800 flex items-center space-x-3">
                        <Avatar name={user?.name} role={user?.role} size="sm" />
                        <div className="overflow-hidden">
                          <p className="text-xs font-bold text-white truncate">{user?.name}</p>
                          <p className="text-[11px] text-slate-400 font-mono">+91 {user?.phone}</p>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full flex items-center space-x-2.5 px-4 py-2.5 text-xs font-medium text-slate-300 hover:bg-indigo-600/15 hover:text-white transition-colors text-left"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center space-x-2.5 px-4 py-2.5 text-xs font-medium text-pink-400 hover:bg-pink-500/10 transition-colors text-left border-t border-slate-800"
                      >
