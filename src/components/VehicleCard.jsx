...
import { Star, MapPin, Gauge, Fuel, Users, Zap, ArrowUpRight, ShieldCheck, Lock } from 'lucide-react';
  ...
  return (
    <div className="glass-card glass-card-hover rounded-2xl overflow-hidden flex flex-col justify-between group">
      ...
      <div className="relative h-52 overflow-hidden bg-slate-900">
        ...
        <div className="absolute top-3 left-3">
          <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold border backdrop-blur-md flex items-center gap-1 ${getCategoryBadgeClass(vehicle.category)}`}>
            {(vehicle.category === 'EV' || vehicle.category === 'Electric') && <Zap className="w-3 h-3 fill-cyan-400" />}
            {vehicle.category}
          </span>
        </div>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 py-2 px-3 rounded-xl bg-slate-900/60 border border-slate-800/80 text-slate-300 text-[11px]">
          <div className="flex items-center space-x-1.5">
            <Gauge className="w-3.5 h-3.5 text-indigo-400" />
            <span className="truncate">{vehicle.horsepower || 300} HP</span>
          </div>
          <div className="flex items-center space-x-1.5">
        ...
        {/* Security Privacy Masking Box (showing only last 4 digits) */}
        <div className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-[10px] text-slate-400 flex items-center justify-between">
          <span className="flex items-center space-x-1">
            <Lock className="w-3 h-3 text-pink-400" />
            <span>Masked Credentials:</span>
          </span>
          <span className="font-mono text-slate-200">
            Engine {vehicle.maskedEngineNumber || '••••8912'}
          </span>
        </div>
        ...
        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between">
          <div>
            <span className="text-xs text-slate-400 font-normal">Daily Rate</span>
