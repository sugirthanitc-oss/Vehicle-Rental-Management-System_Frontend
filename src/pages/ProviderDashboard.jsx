...
const ProviderDashboard = () => {
  ...
  const pendingRequestsCount = bookingRequests.filter(b => b.approvalStatus === 'Waiting for Approval').length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-700/80 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            <Avatar name={user?.shopName || user?.name} role="provider" size="xl" />
            <div>
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="glass-card px-4 py-3 rounded-xl text-xs font-bold text-slate-300 hover:text-white flex items-center space-x-1.5 border border-slate-700 hover:border-pink-500/40 transition-colors"
            >
        ...
        <div className="flex items-center space-x-3 border-b border-slate-800 pb-4">
          ...
          >
            <span>Customer Booking Requests ({bookingRequests.length})</span>
            {pendingRequestsCount > 0 && (
              <span className="ml-2 px-2 py-0.5 rounded-full text-[10px] bg-pink-500 text-white font-extrabold animate-pulse">
          ...
          <div className="space-y-6">
            {vehicles.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {vehicles.map((v) => (
                  <div key={v._id} className="glass-card p-5 rounded-3xl border border-slate-800 space-y-4 relative">
                    ...
                    <div className="relative h-40 rounded-2xl overflow-hidden bg-slate-900">
                      ...
                      <span className={`absolute top-2.5 right-2.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${
                        v.status === 'Available'
                    ...
                    <div className="p-3 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-1.5 text-[11px]">
                      <div className="flex justify-between text-slate-300">
                        ...
                        <span className="font-mono text-indigo-300 font-bold">{v.engineNumber}</span>
                      </div>
                      <div className="flex justify-between text-slate-300">
                        <span className="text-slate-500">Chassis No:</span>
                        <span className="font-mono text-pink-300 font-bold">{v.chassisNumber}</span>
                      </div>
                      <div className="flex justify-between text-slate-300">
                        <span className="text-slate-500">RC Book No:</span>
                        <span className="font-mono text-cyan-300 font-bold">{v.rcBookNumber}</span>
                      </div>
              ...
              bookingRequests.map((b) => (
                <div key={b._id} className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Customer: <strong className="text-slate-200">{b.user?.name}</strong> • Mobile: <strong className="text-indigo-400">+91 {b.user?.phone}</strong></p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-xs">
                    ...
                    <div>
                      <span className="text-slate-500 block text-[11px]">Pickup Location ("From"):</span>
                      <span className="font-medium text-slate-200">{b.pickupLocation}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">Destination ("To"):</span>
                    ...
                    {b.approvalStatus === 'Waiting for Approval' && (
                      <div className="flex items-center space-x-3 ml-auto">
                        ...
                        <button
                          ...
                          className="btn-neon px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-lg shadow-indigo-500/20 flex items-center space-x-1.5"
                        >
