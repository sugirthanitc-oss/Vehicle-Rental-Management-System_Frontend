      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-700/80 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            <Avatar name={user?.name} role="customer" size="xl" />
            <div>
              ...
              <p className="text-xs text-slate-400 flex flex-wrap items-center gap-3 mt-1.5">
                <span>Mobile: <strong className="text-indigo-400 font-mono">+91 {user?.phone}</strong></span>
                {user?.email && <span>Email: <strong className="text-slate-300">{user?.email}</strong></span>}
                {user?.drivingLicenseNumber && (
                  <span>License: <strong className="text-pink-400 font-mono">{user?.drivingLicenseNumber}</strong></span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="glass-card px-4 py-2.5 rounded-xl text-xs font-bold text-slate-300 hover:text-white flex items-center space-x-1.5 border border-slate-700 hover:border-indigo-500/40 transition-colors"
            >
            ...
            <button
              ...
              className="glass-card p-2.5 rounded-xl text-slate-400 hover:text-white border border-slate-700 hover:border-indigo-500/40 transition-colors"
              title="Refresh Data"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="glass-card p-6 rounded-3xl border border-emerald-500/30 flex items-center space-x-4 bg-emerald-500/5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 font-medium">Approved Rentals</p>
            </div>
          </div>
          ...
          <div className="glass-card p-6 rounded-3xl border border-pink-500/30 flex items-center space-x-4 bg-pink-500/5">
            ...
            <div>
              <p className="text-2xl font-extrabold text-pink-400">${totalSpent}</p>
              <p className="text-xs text-slate-300 font-medium">Total Amount Paid</p>
            </div>
          </div>
        </div>

        {/* Bookings Section with Clean Status Filter Tabs */}
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <LayoutDashboard className="w-5 h-5 text-indigo-400" />
              <span>Rental Bookings & Approval Status</span>
            </h2>
                ...
                return (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                      active
                        ? 'bg-gradient-to-r from-indigo-600 to-pink-500 text-white shadow-md shadow-indigo-500/25 scale-105'
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2].map((i) => (
                <div key={i} className="glass-card h-40 rounded-3xl animate-pulse p-4" />
              ))}
            </div>
          ) : filteredBookings.length === 0 ? (
            <div className="glass-card p-12 rounded-3xl border border-slate-800 text-center space-y-3">
              <Calendar className="w-12 h-12 text-slate-600 mx-auto" />
              <h3 className="text-base font-bold text-white">No Bookings in "{activeTab}"</h3>
              <p className="text-xs text-slate-400">You haven't placed any rental bookings matching this status yet.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredBookings.map((b) => (
                <div key={b._id} className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
                    <div className="flex items-center space-x-4">
                      <img
                        ...
                        alt={b.vehicle?.title}
                        className="w-20 h-16 object-cover rounded-2xl border border-slate-700"
                      />
                      <div>
                        <div className="flex items-center space-x-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 text-xs">
                    <div>
                      <span className="text-slate-500 block text-[11px]">Driving License Verification:</span>
                      <span className="font-mono font-bold text-indigo-300">{b.drivingLicenseNumber}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">Rental Dates:</span>
                      <span className="font-semibold text-slate-200">
                        {new Date(b.startDate).toLocaleDateString()} - {new Date(b.endDate).toLocaleDateString()} ({b.totalDays} Days)
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-500 block text-[11px]">Pickup Verification Code:</span>
                      {b.verificationCode ? (
                        <span className="font-mono text-emerald-400 font-extrabold text-sm">{b.verificationCode}</span>
                      ) : (
                        <span className="text-amber-400 italic text-[11px]">Generates upon Provider Approval</span>
                      )}
                    </div>
                  </div>
